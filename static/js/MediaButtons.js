...
class MediaButtons {
  constructor (evtSys, mediaLibrary) {
    ...
    react('[role="play"]', 'click', (ev) => { this.play(ev); });
    react('[role="toggle-player"]', 'click', (ev) => { this.nowPlaying(ev); });
    react('[role="next"]', 'click', (ev) => { this.next(ev); });
    react('[role="prev"]', 'click', (ev) => { this.prev(ev); });
    react('[role="shuffle"]', 'click', (ev) => { this.shuffle(ev); });
    react('[role="update-library"]', 'click', (ev) => { this.updateLibrary(ev); });
  ...
  nowPlaying(ev) { this.mediaLibrary.toggleNowPlaying(false); }

  next(ev) { this.mediaLibrary.nextSong(); }

  prev(ev) { this.mediaLibrary.prevSong(); }
